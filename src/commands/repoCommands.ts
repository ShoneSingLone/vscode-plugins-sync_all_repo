...
export function registerRepoCommands(
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand(CMD("refresh"), async () => {
			await repoManager.refreshDiscovery();
			OutputManager.getInstance().info("仓库列表已刷新");
		})
	);

	// 添加远程（对当前编辑器所属仓库操作）
	ctx.subscriptions.push(
		vscode.commands.registerCommand(CMD("addRemote"), async () => {
	...
	ctx.subscriptions.push(
		vscode.commands.registerCommand(CMD("manageRemotes"), async () => {
			const repo = await _pickRepo(repoManager);
			if (!repo) {
				return;
			}
			...
			const items: Array<vscode.QuickPickItem & { action: string; remoteName: string }> = [];
	...
	// 打开仓库终端
	ctx.subscriptions.push(
		vscode.commands.registerCommand(CMD("openTerminal"), async (uri?: vscode.Uri) => {
	...
	// 在资源管理器中显示仓库
	ctx.subscriptions.push(
		vscode.commands.registerCommand(CMD("revealInExplorer"), async (uri?: vscode.Uri) => {
			const repo = await _pickRepo(repoManager, uri);
			if (repo) {
